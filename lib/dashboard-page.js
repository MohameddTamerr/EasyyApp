      ...
      function renderProjects(submissions) {
        ...
        listEl.innerHTML = submissions.map((submission) => {
          ...
          const logoDownload = submission.logoFileDataUrl
            ? `<div class="asset-actions"><a class="asset-link" href="${escapeHtmlClient(submission.logoFileDataUrl)}" download="${escapeHtmlClient(submission.logoFileName || `logo-${submission.id}.png`)}">Download Uploaded Logo</a></div>`
            : "";
