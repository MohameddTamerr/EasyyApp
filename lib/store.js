...
const IS_VERCEL_RUNTIME = Boolean(process.env.VERCEL || process.env.VERCEL_ENV || process.env.NOW_REGION);

let blobSdkPromise;

function createStorageConfigError() {
  const error = new Error("Storage is not configured for production. Add BLOB_READ_WRITE_TOKEN in Vercel and redeploy.");
...
function assertStorageReady() {
  if (!useBlobStorage() && IS_VERCEL_RUNTIME) {
...
function createSubmissionRecord(payload) {
  ...
  return normalizeSubmission({
    ...
    logoFileDataUrl: String(payload.logoFileDataUrl || "").trim(),
...
function validateSubmissionRecord(submission) {
  const emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(submission.clientEmail);
  const phoneDigits = submission.phone.replace(/\D/g, "");
  const phoneOk = !submission.phone || (/^[\d\s()+-]+$/.test(submission.phone) && phoneDigits.length >= 7 && phoneDigits.length <= 15);
  const websiteOk = !submission.currentWebsite || (() => {
    try {
      const parsed = new URL(/^https?:\/\//i.test(submission.currentWebsite) ? submission.currentWebsite : `https://${submission.currentWebsite}`);
      return Boolean(parsed.hostname && parsed.hostname.includes("."));
    } catch {
      return false;
    }
  })();
  const logoOk = !submission.logoFileDataUrl || /^data:image\/[a-zA-Z0-9.+-]+;base64,/.test(submission.logoFileDataUrl);

  return Boolean(
    submission.clientName &&
    submission.clientName.length >= 2 &&
    emailOk &&
    phoneOk &&
    websiteOk &&
    logoOk &&
    Array.isArray(submission.summaryItems) &&
    submission.summaryItems.length
  );
}
